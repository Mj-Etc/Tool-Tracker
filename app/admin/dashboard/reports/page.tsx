...
import { format, isSameDay, startOfWeek, endOfWeek, startOfMonth, endOfMonth, eachDayOfInterval } from "date-fns";
...
import { Transaction, Item, Stats, ReportMode } from "@/components/reports/types";
...
export default function ReportsPage() {
  ...
  const { startDate, endDate } = useMemo(() => {
    if (reportMode === "daily") return { startDate: selectedDate, endDate: selectedDate };
    if (reportMode === "weekly") return { startDate: startOfWeek(selectedDate), endDate: endOfWeek(selectedDate) };
    if (reportMode === "monthly") return { startDate: startOfMonth(selectedDate), endDate: endOfMonth(selectedDate) };
    return { startDate: undefined, endDate: undefined };
  }, [reportMode, selectedDate]);

  const statsUrl = reportMode === "overall" 
    ? `/api/stats?overall=true` 
    : `/api/stats?startDate=${format(startDate!, "yyyy-MM-dd")}&endDate=${format(endDate!, "yyyy-MM-dd")}`;
  
  const transUrl = reportMode === "overall" 
  ...
  const revenue = useMemo(() => transactions?.reduce((sum, t) => sum + Number(t.totalAmount), 0) || 0, [transactions]);
  
  const { cost, profit, margin } = useMemo(() => {
    let cost = 0;
    transactions?.forEach(t => t.items.forEach(i => { cost += (Number(i.item.costPrice) * i.quantity); }));
  ...
  const chartData = useMemo(() => {
    if (!transactions || reportMode === "overall" || reportMode === "daily") return [];
    const days = eachDayOfInterval({ start: startDate!, end: endDate! });
    return days.map(day => ({
      date: format(day, "MMM dd"),
      revenue: transactions.filter(t => isSameDay(new Date(t.createdAt), day)).reduce((sum, t) => sum + Number(t.totalAmount), 0)
    }));
    ...
    <div className="flex flex-col gap-8 p-6 max-w-7xl mx-auto w-full min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 transition-[height] duration-300 ease-in-out">
        <div className="space-y-1 flex-1 min-w-0">
          ...
          <h1 className="text-4xl font-bold tracking-tight transition-all duration-300">
            {reportMode === "overall" ? "Lifetime Performance" : 
             reportMode === "weekly" ? "Weekly Performance" :
             reportMode === "monthly" ? "Monthly Performance" :
             (isToday ? "Today's Performance" : "Daily Report")}
          </h1>
          <p className="text-muted-foreground transition-all duration-300">
            {reportMode === "overall" ? "All-time aggregated business metrics" : 
             reportMode === "weekly" ? `${format(startDate!, "MMM d")} - ${format(endDate!, "MMM d, yyyy")}` :
             reportMode === "monthly" ? format(selectedDate, "MMMM yyyy") :
        ...
        <div className="flex flex-wrap items-center gap-3 shrink-0">
          <ToggleGroup type="single" value={reportMode} onValueChange={(v) => v && setReportMode(v as any)} className="border p-1 bg-muted/50 rounded-lg">
            ...
            <ToggleGroupItem value="weekly" className="px-4 text-xs font-bold uppercase tracking-wider data-[state=on]:bg-background">Weekly</ToggleGroupItem>
            <ToggleGroupItem value="monthly" className="px-4 text-xs font-bold uppercase tracking-wider data-[state=on]:bg-background">Monthly</ToggleGroupItem>
          ...
          <Badge variant="secondary" className="h-10 px-4 rounded-md border text-xs font-mono shrink-0">
            SYS_REF: {reportMode === "overall" ? "00000000" : 
                      reportMode === "weekly" ? `W${format(selectedDate, "ww")}${format(selectedDate, "yyyy")}` :
                      reportMode === "monthly" ? `M${format(selectedDate, "MM")}${format(selectedDate, "yyyy")}` :
                      dateStr.replace(/-/g, '')}
          </Badge>
        </div>
      </div>
      ...
      {isDataLoading ? (
        ...
        <div className="space-y-8 animate-in fade-in duration-500">
          <KpiCards revenue={revenue} stats={stats} reportMode={reportMode} margin={margin} />
          ...
          <Tabs defaultValue="overview" className="space-y-6">
            ...
            <TabsContent value="overview" className="space-y-6 outline-none">
              <RevenueTrendChart reportMode={reportMode} chartData={chartData} />
            ...
            <TabsContent value="transactions" className="outline-none">
              <TransactionJournal reportMode={reportMode} selectedDate={selectedDate} transactions={transactions} />
            </TabsContent>
